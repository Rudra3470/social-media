     ...
     <>
      {
        ...
        {
            show && <Model value={followersData} title={"Followers"} setShow={setShow} />
        }

        {
            show1 && <Model value={followingsData} title={"Followings"} setShow={setShow1} />
        }
      <div className="bg-white flex justify-between gap-4 p-8 rounded-lg shadow-md max-w-md">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input type="custom-input" style={{width: "80px"}} value={name}onChange={e=>setName(e.target.value)}placeholder="Enter Name" required />
          ...
          <p className="text-gray-500 text-sm cursor-pointer" onClick={()=>setShow(true)}>{user.followers.length}follower</p>
          <p className="text-gray-500 text-sm cursor-pointer" onClick={()=>setShow1(true)}>{user.followings.length}following</p>
      ...
      {showUpdatePass && 
      <form onSubmit={updatePassword} className="flex justify-center items-center flex-col bg-white p-2 rounded-sm gap-4">
