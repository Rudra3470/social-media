    ...
    <div className="bg-gray-100 flex items-center justify-center pt-3 pb-14">
        <SimpleModal isOpen={showModal} onClose={closeModal}>
            <div className="flex flex-col items-center justify-center gap-3">
                ...
                <button onClick={deleteHandler} className="bg-red-400 text-white py-1 px-3 rounded-md" disabled={loading}>{loading?<LoadingAnimation/> : "Delete"}</button>
            </div>
        </SimpleModal>
        ...
        <div className="mb-4">
           {showInput?<>
           <input className="custom input" style={{width: "150px"}} type="text" placeholder="Enter Caption" value={caption} onChange={e=>setCaption(e.target.value)} required/>
           <button onClick={updateCaption} className="text-sm bg-blue-500 text-white px-1 py-1 rounded-md" disabled={captionLoading}>{captionLoading?<LoadingAnimation/>:"Update Caption"}</button>
        ...
        <div className="mb-4">
            {type==="post"?<img src={value.post.url} alt="" className="object-cover rounded-md"/>:<video src={value.post.url} alt="" className="w-[450px] h-[600px] object-cover rounded-md" autoPlay controls/>}
        </div>
        ...
        </div>
        {
            show && <form onSubmit={addCommentHandler} className="flex gap-3">
                <input type="text" className="custom-input" placeholder="Enter Comment" value={comment} onChange={e=>setComment(e.target.value)}/>
