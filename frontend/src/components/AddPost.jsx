    ...
    <div className="bg-gray-100 flex items-center justify-center pt-3 pb-5">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md">
            <form onSubmit={submitHandler} className="flex flex-col gap-4 items-center justify-between mb-4">
                <input type='text' className="custom-input" placeholder="Enter Caption" value={caption} onChange={e=>setCaption(e.target.value)}/>
                <input type='file' className="custom-input" accept={type==="post"?"image/*":"video/*"} onChange={changeFileHandler}required/>
                {
                    filePrev && <>
                    {type==='post'?<img src={filePrev} alt=""/>:<video 
